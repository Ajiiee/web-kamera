  ...
  <div class="gallery">
    ...
    <div class="content">
      ...
      <form action="login.php" method="post">
        <?php $idUser = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM tb_admin")) ?>
        <input type="hidden" name="userID" value="<?= $idUser ?>">
        <input type="hidden" name="productID" value="<?= $p['product_id'] ?>">
        <button type="submit" name="addToCart" class="buy-1"  onclick="return confirm('SIlahkan Login Terlebih Dahulu')">Masukan ke keranjang</button>
      </form>
    </div>
    <?php endforeach; ?>
  </div>
