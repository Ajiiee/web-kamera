<!DOCTYPE html>
    ...
    <head>
        ...
        <link rel="stylesheet" tyype="text/css" href="cart.css">
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="cart">
                <div class="products">
                    <?php
                    $idUser = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM tb_admin WHERE username = '" . $_SESSION["login-kamera"] . "'"))["admin_id"];

                    $cek = mysqli_query($conn, "SELECT * FROM tb_cart WHERE id_user = $idUser AND status = 0");
                    ...
                    ?>
                        ...
                        <?php endforeach; ?>
                    <?php else : ?>
                        <h3>Kerjanjang Kosong</h3>
                    <?php endif; ?>
                </div>
                <div class="cart-total">
                    <?php
                        $carts = mysqli_query($conn, "SELECT * FROM tb_cart WHERE id_user = $idUser AND status = 0");
                        $totalHarga = 0;
                        $totalBarang = 0;
                    ...
                    <form action="confirm.php" method="post">
                        <input type="hidden" value="<?= $_SESSION['login-kamera'] ?>" name="username">
                        <button type="submit" href="confirm.php" name="submit" style="padding: 10px 5px; background-color:aqua">Proceed to Checkout</button>
                    </form>
                </div>
            </div>
        </div>
